...
%	{ \draw[#1] ($(#2)+({#5*cos(#3)},{#5*sin(#3)})$) arc (#3:#4:#5) node[below]{$\theta$}; }
	...
	\draw[dashed,very thin] (-2,0) node[below] {$x$} -- (2,0);
	\draw[dashed,very thin] (0,-1.5) -- (0,2) node[left]{$y$};
	
	\draw[->] (0,0) -- node[above right]{$\vec{N}$} (0,0.866025404);
	\draw[->,gray] (0,0) -- (-0.5,0) node[above=3pt]{$P_x$};
	\draw[->,gray] (0,0) -- (0, -0.866025404) node[right]{$P_y$};
	\draw[->] (0,0) -- node[below right]{$\vec{F}_a$} (0.5,0);
	\draw[dotted] (-0.5,0) -- (-0.5,-0.866025404);
	\draw[dotted] (-0.5,-0.866025404) -- (0, -0.866025404);
...
Verificamos para o \textbf{eixo $y$} que a força normal deve ser equilibrada pela componente $y$ da força peso, pois o objeto não se desloca no eixo $y$ e, portanto, sua aceleração deve ser zero. Já para o \textbf{eixo $x$}, verificamos que
\begin{equation}
	P_x - F_a = ma_x.
\end{equation}
%
